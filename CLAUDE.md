...
- **`Etc/UTC` event render passthrough** — `enrich_events` does NOT convert `start` to local time. Events whose Morgen `timeZone` is `Etc/UTC` are displayed verbatim as UTC wall-clock (off by the local offset from what the user actually scheduled). Don't add conversion in gm — this is Morgen's API contract and downstream callers expect raw passthrough. The skill (`.claude/skills/gm/SKILL.md`) documents the cross-check (UID `T<HHMMSS>Z` token, if present, beats `start`). Observed against Morgen API as of v0.23.9 / 2026-05.
