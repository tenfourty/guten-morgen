...
- **Local-time render conversion** — `enrich_events` converts each event's `start`/`end` to an offset-qualified ISO string in the caller's local zone via `to_local_iso` (e.g. an `America/New_York` or `Etc/UTC` event renders as `…+02:00`) and normalizes `timeZone` to the local zone. Floating/all-day events (no `timeZone`) pass through unchanged, and `to_local_iso` degrades to the raw `start` on an unknown zone / unparseable value so one bad event can't break a listing. `--raw-times` (→ `_raw_times_active()` → `enrich_events(raw_times=True)`) opts out and returns Morgen's raw per-event wall-clock. `compute_free_slots` (gm availability) converts through the same `_to_local_aware` helper. Downstream callers and the skill (`.claude/skills/gm/SKILL.md`) read `start` as already-local.
- **`--due` time round-trip lossy** — `_normalize_due()` passes any `HH:MM:SS` component through unchanged on the wire, but Morgen's storage discards it; `tasks get` always returns date-only / end-of-day. For tasks needing a specific time, use `tasks schedule <id> --start <ISO>` (creates a linked calendar event) instead of `--due`.
- **Clearing a due date** — `tasks update --clear-due` (or `--due ""`) sends `due: null`. ⚠️ Empty-string-clears is a footgun: a scripted `--due "$VAR"` with an accidentally-empty `$VAR` silently clears the due date rather than erroring — prefer `--clear-due` in scripts. `--due <value>` + `--clear-due` together is an `invalid_input` error. (`tasks create` differs: an empty `--due` there is a no-op, since there's nothing to clear.)
...
- **Never shadow Python builtins with click command function names** — `@cli.command()` on `def next(...)` binds the module global `next` to a click `Command`, which shadows `builtins.next` for every other function in the module. Calling `next(iter, default)` elsewhere then invokes the Command's `__call__`, re-enters click's parser, and dies with `TypeError: object of type '<X>' has no len()` from `click/parser.py`. Always pair a builtin-colliding CLI name with an explicit click name + safe Python name: `@cli.command("next")` + `def gm_next(...)`. Enforced by ruff A001.
